...
class LanguageModelConfig(BaseModel):
    ...
    def _validate_api_key(self) -> None:
        ...
        API Key is required when using OpenAI API, AIME API
    ...
    def _validate_auth_type(self) -> None:
        ...
        auth_type must be api_key when using OpenAI, AIME and
        can be either api_key or azure_managed_identity when using AOI.
        auth_type must be none when using BGE embeddings.
        ...
        if self.auth_type == AuthType.AzureManagedIdentity and (
            ...
            or self.type == ModelType.AIMEChat
            or self.type == ModelType.AIMEEmbedding
        ):
        ...
        if self.type == ModelType.BGEEmbedding and self.auth_type != AuthType.None_:
            msg = f"auth_type must be none for BGE embeddings. Please rerun `graphrag init` and set the auth_type to none."
            raise ConflictingSettingsError(msg)
    ...
    def _validate_encoding_model(self) -> None:
        ...
        KeyError
            If the model name is not recognized.
        """
        # Skip encoding model validation for BGE embeddings since they don't use tiktoken
        if self.type == ModelType.BGEEmbedding:
            return
    ...
    def _validate_api_base(self) -> None:
        ...
        Required when using AOI or AIME.
    ...
    def _validate_model(self):
        ...
        ApiKeyMissingError
            If the API key is missing and is required.
        ConflictingSettingsError
            If the settings conflict with each other.
        """
        self._validate_type()
